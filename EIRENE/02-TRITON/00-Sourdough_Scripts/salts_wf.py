...
# Updated to include function that returns only weight fractions
...
class Salt(object):
    ...
    def chloride_density_equation_BoLiShengDai(self, x: float, tempK: float) -> float:
        """Density calculation using Equation 4 from https://doi.org/10.1016/j.molliq.2019.112184
    ...
    def wf_gen(self):
        if not self.wflist:  # Generate list of isotopic weight fractions
            self._isotopic_fractions()
        #dens = self.densityK(tempK)*dens_mod
        wf_mat = []
        for w in self.wflist:
            wf_mat.append(w.wf)
        return wf_mat
