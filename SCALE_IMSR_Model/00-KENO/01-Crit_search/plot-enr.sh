...
f(x) = a*x +b
fit [2.2:2.4] f(x) 'data-temp.out' u 1:(\$2-1)*1e5/\$2:(\$3*1e5)  yerrors via a,b
...
plot [Xmin:Xmax] 'data-temp.out' u 1:(\$2-1)*1e5/\$2:(\$3*1e5) w e ls 7 ps .5 lc "blue" notit, [2.2:2.4] f(x) ls 1 lw 0.4 lc "navy" tit sprintf(" slope = %5.0f {/Symbol \261} %5.0f pcm/Uenr\%", a, a_err)
